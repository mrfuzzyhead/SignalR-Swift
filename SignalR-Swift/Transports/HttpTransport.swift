...
public class HttpTransport: ClientTransportProtocol {
    ...
    public func negotiate(connection: ConnectionProtocol, connectionData: String?, completionHandler: ((NegotiationResponse?, Error?) -> ())?) {
        ...
        encodedRequest.validate().responseJSON { (response: AFDataResponse<Any>) in
    ...
    public func send(connection: ConnectionProtocol, data: Any, connectionData: String?, completionHandler: ((Any?, Error?) -> ())?) {
        ...
        request.validate().responseJSON { (response: AFDataResponse<Any>) in
            switch response.result {
            case .success(let result):
    ...
    public func abort(connection: ConnectionProtocol, timeout: Double, connectionData: String?) {
        ...
        // TODO: - this is throwing an exception => encodedRequest.request!.url!.absoluteString
        if let absoluteUrl = encodedRequest.request?.url?.absoluteString {
            let request = connection.getRequest(url: absoluteUrl, httpMethod: .post, encoding: URLEncoding.httpBody, parameters: nil)
